...
function Hero() {
  ...
  useEffect(() => {
    const popularMoviesUrl = Endpoints.popular;
     axios.get(popularMoviesUrl).then((response) => {
        const movies = response.data.results;
        const randumMovie = movies[Math.floor(Math.random() * movies.length)];
        setMovie(randumMovie);
      })
      .catch((error) => {
  ...
  return <div className="w-full h-[500px] lg:h-[850px]">
